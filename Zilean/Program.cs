...
namespace Zilean
{
    class Program
    {
        ...
        private static void Obj_AI_Base_OnProcessSpellCast2(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (sender == null || !Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Harass) ||  (sender.Hero == Champion.Yasuo && sender.Mana >= 75))
            {
               return;
            }
            ...
            if (Q.IsReady() && sender.IsValidTarget(900) && !sender.IsInvulnerable && args.Target != CurrentTarget && !sender.IsDashing() && sender == CurrentTarget)
            {
                ...
                if (args.End.Distance(Player.Instance.Position) >= 100 || args.SData.TargettingType == SpellDataTargetType.Unit)
                {
                    ...
                    {
                        if (!miscMenuT[args.SData.Name].Cast<CheckBox>().CurrentValue)
                        {
        ...
        private static void Obj_AI_Base_OnBasicAttack(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            var bc = (sender.Instance.Mana);
            if (sender == null || (!flags.HasFlag(Orbwalker.ActiveModes.Harass)) || (sender.Hero == Champion.Yasuo && sender.Mana >= 75))
            {
               return;
            }
            CurrentTarget = TargetSelector.GetTarget(Q.Range, DamageType.Magical);
        ...
        static void Harass1()
        {
          ...
          //  }
            ...
            var Minions = EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Both,target.ServerPosition, 300);
            foreach (var Minion in Minions)
            if(Player.Instance.Distance(Minion.ServerPosition) <= 900 && Minion.HasBuff("ZileanQEnemyBomb"))
            {
